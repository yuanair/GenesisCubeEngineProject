...
namespace GenesisCube
{
	...
	public:
		...
		/// formatter - 日志输出格式类
		FLogger(TString dir, const TString &filename, const FLoggerFormat *formatter);
	...
	public:
		...
		/// 消息框
		/// \param exception 异常
		static void MessageBoxFromException(const EException &exception);
		
		///
		/// 消息框
		/// \param exception 异常
		static void MessageBoxFromException(const std::exception &exception);
	...
	/// \param hr HRESULT
	/// \param message 消息
	void ThrowIfFailed(HRESULT hr, const TString &message = TEXT("DirectX error"));
	
	///
	/// 抛出异常
	/// \param hr HRESULT
	/// \param message 消息
	void ThrowHResult(HRESULT hr, const TString &message = TEXT("DirectX error"));
